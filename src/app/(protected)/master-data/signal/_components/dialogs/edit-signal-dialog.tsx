...
interface EditSignalDialogProps {
  ...
  signal: Signal | null;
  onSubmit: (data: Omit<Signal, "id" | "createdAt">) => void;
}
...
export function EditSignalDialog({
  ...
  useEffect(() => {
    if (signal) {
      ...
      form.reset({
        emitenCode: signal.emitenCode,
        emitenName: signal.emitenName,
        type: signal.type,
    ...
    <Dialog open={open} onOpenChange={onOpenChangeHandler}>
      <DialogContent className="max-w-4xl">
        <DialogHeader>
          ...
          <DialogDescription>Edit sinyal investasi yang sudah ada.</DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form
